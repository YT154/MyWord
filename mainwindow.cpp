...
{
    QMessageBox::about(this,tr("关于"),tr("这是一个基于qt6.12实现的字处理器软件\r\n具备类似 微软Offic Word的功能"));
}
...
void MainWindow::initMainWindow()
{
    ...
    refreshMenus();
    connect(ui->mdiArea,SIGNAL(subWindowActivated(QMdiSubWindow*)),this,SLOT(refreshMenus()));//当有活动文档子窗口时刷新菜单

    //将段落菜单下的各对齐方式功能项加入同一个菜单项组，以便程序运行的任一时刻，用户能且只能选中其中一项
...
void MainWindow::addSubWndListMenu()
{
    ...
    if(!wnds.isEmpty())
        ui->menu_W->addSeparator();//如果有活动子窗口，则显示分隔条

    //遍历各子窗口，显示所有当前已打开的文档子窗口项
    for(int i=0;i<wnds.size();++i)
    {
        MyChildWnd* childwnd=qobject_cast<MyChildWnd*>(wnds.at(i)->widget());//返回当前内部的小部件
        ...
        //将当前活动的子窗口设置为勾选状态
        menuitem_act->setCheckable(childwnd==activateChildWnd());
    ...
    formatEnabled();//使字体菜单下的功能可用
}

void MainWindow::docNew()
{
    MyChildWnd * childWnd=new MyChildWnd; //创建MyChildWnd部件
    ui->mdiArea->addSubWindow(childWnd); //向多文档区域添加子窗口，childWnd为中心部件
...
void MainWindow::refreshMenus()//根据是否有活动子窗口来设置各菜单项对应的动作时候可用
{
    //至少有一个文档子窗口打开的情况
...
MyChildWnd* MainWindow::activateChildWnd()//判断是否有活动子窗口
{
    //若有活动文档窗口则将其内的中心部件转换为MyChiladWnd类型，若没有直接返回0
...
{
    QString docName=QFileDialog::getOpenFileName(this,tr("打开"),QString(),tr("HTML文档(*.htm *.html);;所有文件(*.*);;Images (*.png *.xpm *.jpg);;Text files (*.txt);;XML files (*.xml)"
));//返回用户选择的现有文件
    if(!docName.isEmpty())
    {
        QMdiSubWindow* exist=findChildWnd(docName);//遍历所有子窗口，判断文档是否已经被打开
        if(exist)//为真时，表示文档已经打开
        {
...
QMdiSubWindow* MainWindow::findChildWnd(const QString &docName)
{
    ...
    foreach(QMdiSubWindow* wnd,ui->mdiArea->subWindowList())//遍历整个多文档区域
    {
...
{
    if(activateChildWnd())
        activateChildWnd()->undo();//撤销
}

void MainWindow::docRedo()
{
    if(activateChildWnd())
        activateChildWnd()->redo();//重做
}

void MainWindow::docCut()
{
    if(activateChildWnd())
        activateChildWnd()->cut();//剪切
}

void MainWindow::docCopy()
{
    if(activateChildWnd())
            activateChildWnd()->copy();//复制
}

void MainWindow::docPaste()
{
    if(activateChildWnd())
            activateChildWnd()->paste();//粘贴
}
...
void MainWindow::on_sizeComboBox_editTextChanged(const QString &arg1)
{
    textSize(arg1);
}
